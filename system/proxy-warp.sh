...
# ===== IOS DOMAIN MATCH FIX =====
# Add sniffing INSIDE each client inbound object.
# Existing sniffing blocks are left untouched. The API inbound is skipped.
# This operates line-by-line and preserves the existing JSONC/comments.
ensure_inbound_sniffing_file() {
  local f="$1"
  awk '
    BEGIN {
      inInbounds=0
    ...
    function print_obj(    i, last, ind, j, line, c, trimmed, addcomma) {
      # Find whether this inbound already has sniffing.
      if (hasSniff==0 && isApi==0) {
        # Find closing-brace indentation and the last property line.
        last=n-1
        while (last>0 && buf[last] ~ /^[ \t]*$/) last--

        match(buf[n], /^[ \t]*/)
        ind=substr(buf[n], RSTART, RLENGTH) "  "

        # The previous property must end with a comma before sniffing.
        trimmed=buf[last]
        if (trimmed !~ /,[ \t]*$/ && trimmed !~ /^[ \t]*{/)
          buf[last]=buf[last] ","
        ...
        print ind "\"sniffing\": {"
        print ind "  \"enabled\": true,"
        print ind "  \"destOverride\": ["
        print ind "    \"http\","
        print ind "    \"tls\""
        print ind "  ],"
        print ind "  \"routeOnly\": true"
        print ind "},"
...

    {
      ...
      if (inObj==1) {
        # Detect API inbound by its tag/protocol anywhere in the object.
        if (line ~ /"tag"[ \t]*:[ \t]*"api"/)
          isApi=1
        ...
        # Closing line of this top-level inbound object.
        if (depth==1 && line ~ /^[ \t]*}[ \t]*,?[ \t]*$/) {
          n++
          buf[n]=line
          print_obj()
...
apply_mode_all() {
  ...
  for f in "${CFG_LIST[@]}"; do
    ...
    ensure_inbound_sniffing_file "$f"
